...
# Яг одоогийн үнийг татах — df хасагдахаас өмнө хадгална
try:
    fast = yf.download(ticker, period='5d', progress=False, auto_adjust=True)
    current_price = float(fast['Close'].dropna().iloc[-1])
    prev_price    = float(fast['Close'].dropna().iloc[-2])
    today_str     = fast.index[-1].strftime('%Y-%m-%d')
except Exception:
...
# Таамаглалын input: хасагдахаас өмнөх хамгийн сүүлийн мөр (5 өдрийн өмнөх)
predict_row      = df[feature_cols].iloc[[-6]]   # 5 өдрийн өмнөх өгөгдөл
predict_date     = df.index[-6]                  # тэр өдрийн огноо
predict_price    = float(df['close'].iloc[-6])   # тэр өдрийн үнэ
target_date      = df.index[-1]                  # +5 өдрийн дараах огноо (өнөөдөр)
actual_price     = float(df['close'].iloc[-1])   # өнөөдрийн үнэ (хариулт)
...
# ============================================================
print("\n" + "="*60)
print(f"  🔮 5 ӨДРИЙН ТААМАГЛАЛ — {ticker}")
print("="*60)
print(f"  📅 Input огноо      : {predict_date.strftime('%Y-%m-%d')}  (5 өдрийн өмнө)")
print(f"  💰 Input үнэ        : {predict_price:,.2f} {currency}")
print(f"  📅 Таамаглалын огноо: {target_date.strftime('%Y-%m-%d')}  (өнөөдөр)")
print(f"  💰 Өнөөдрийн үнэ   : {actual_price:,.2f} {currency}")
real_change = (actual_price - predict_price) / predict_price * 100
real_dir = "📈 ӨССӨН" if actual_price > predict_price else "📉 БУУРСАН"
print(f"  ✅ Бодит өөрчлөлт   : {real_change:+.2f}%  {real_dir}")
print("-"*60)
...
print("-"*60)
# Таамаглал зөв байсан эсэхийг шалгах
predicted_up = weighted_prob >= 0.5
actual_up    = actual_price > predict_price
correct      = predicted_up == actual_up
print(f"  🎯 Таамаглал зөв эсэх: {'✅ ЗӨВТ' if correct else '❌ БУРУУ'}")
print("="*60)
